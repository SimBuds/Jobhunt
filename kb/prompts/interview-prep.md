...
2. **Stage-aware emphasis.** The `stage` variable shifts what to prioritise:
   - `screen` (recruiter, ~30 min): culture-fit + logistics + 60-second
     background story. Lighter on technical depth. Surface salary expectations,
     timezone, start date, work authorisation.
   - `assessment` (take-home or live coding): what they're likely to test
     given the JD, time-budget framing, what to optimise for, anti-patterns
     to avoid.
   - `hm` (hiring manager): deeper technical, project ownership, scope
     questions, how decisions get made. Bring sharper questions to ask back.
   - `onsite` (final round): team dynamics, multi-stakeholder anchors,
     decision-making style, post-offer logistics.
...
5. **Honest gaps, no defensive volunteering.** The `honest_gaps` array is
   ...
   production, but I've used adjacent Y for [concrete project]." Be honest,
